export type RelayStationFixture = {
  slug: string;
  name: string;
  domain: string;
  websiteUrl: string;
  description: string;
  billingModes: string[];
  paymentMethods: string[];
  minimumTopUpAmount: number | null;
  minimumTopUpCurrency: string | null;
  supportChannels: string[];
  channelType: "official_direct" | "third_party_relay";
  supportedProviders: string[];
  hasPublicPricing: boolean;
  hasTrialCredit: boolean;
  hasReferralProgram: boolean;
  referralUrl: string | null;
  couponCode: string | null;
  isSponsored: boolean;
  isVerified: boolean;
  status: "draft" | "published" | "hidden" | "archived";
  riskLevel: "unknown" | "low" | "medium" | "high";
  riskTags: string[];
  sourceUrl: string;
  lastCheckedAt: string;
};

export type RelayModelPriceFixture = {
  relaySlug: string;
  modelSlug: string;
  routeName: string | null;
  billingType: string;
  modelMultiplier: number | null;
  completionMultiplier: number | null;
  groupMultiplier: number;
  routeMultiplier: number;
  inputPricePer1M: number | null;
  outputPricePer1M: number | null;
  currency: string;
  sourceUrl: string | null;
  lastCheckedAt: string | null;
  isCurrent: boolean;
  notes: string | null;
};

export const riskTags = [
  {
    slug: "new-station",
    label: "New station",
    description: "运营时间或社区反馈仍需观察。",
    severity: "warning",
  },
  {
    slug: "no-public-pricing",
    label: "No public pricing",
    description: "公开价格信息不足，使用前需要自行确认。",
    severity: "warning",
  },
  {
    slug: "manual-review",
    label: "Manual review",
    description: "来自人工整理或调研报告，尚未接入自动监控。",
    severity: "info",
  },
  {
    slug: "referral-available",
    label: "Referral available",
    description: "站点可能存在推荐计划，需要明确标注推荐关系。",
    severity: "info",
  },
  {
    slug: "international-payment",
    label: "International payment",
    description: "可能更适合支持 VISA 或 USDT 的用户。",
    severity: "info",
  },
];

export const relayStations: RelayStationFixture[] = [
  {
    slug: "openai-platform",
    name: "OpenAI Platform",
    domain: "platform.openai.com",
    websiteUrl: "https://platform.openai.com/",
    description:
      "OpenAI official API platform for GPT, GPT-4o and o-series model access.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Credit card", "Invoice"],
    minimumTopUpAmount: null,
    minimumTopUpCurrency: "USD",
    supportChannels: ["Docs", "Console"],
    channelType: "official_direct",
    supportedProviders: ["OpenAI"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: true,
    status: "published",
    riskLevel: "low",
    riskTags: ["manual-review"],
    sourceUrl: "https://openai.com/api/pricing/",
    lastCheckedAt: "2026-05-25T00:00:00.000Z",
  },
  {
    slug: "anthropic-console",
    name: "Anthropic Console",
    domain: "console.anthropic.com",
    websiteUrl: "https://console.anthropic.com/",
    description:
      "Anthropic official Console for Claude API usage, billing and key management.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Credit card", "Invoice"],
    minimumTopUpAmount: null,
    minimumTopUpCurrency: "USD",
    supportChannels: ["Docs", "Console"],
    channelType: "official_direct",
    supportedProviders: ["Anthropic"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: true,
    status: "published",
    riskLevel: "low",
    riskTags: ["manual-review"],
    sourceUrl: "https://docs.anthropic.com/en/docs/about-claude/pricing",
    lastCheckedAt: "2026-05-25T00:00:00.000Z",
  },
  {
    slug: "google-gemini-api",
    name: "Google Gemini API",
    domain: "ai.google.dev",
    websiteUrl: "https://ai.google.dev/gemini-api",
    description:
      "Google official Gemini API and AI Studio entry for Gemini model access.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Credit card", "Google Cloud billing"],
    minimumTopUpAmount: null,
    minimumTopUpCurrency: "USD",
    supportChannels: ["Docs", "AI Studio", "Google Cloud"],
    channelType: "official_direct",
    supportedProviders: ["Google"],
    hasPublicPricing: true,
    hasTrialCredit: true,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: true,
    status: "published",
    riskLevel: "low",
    riskTags: ["manual-review"],
    sourceUrl: "https://ai.google.dev/gemini-api/docs/pricing",
    lastCheckedAt: "2026-05-25T00:00:00.000Z",
  },
  {
    slug: "deepseek-platform",
    name: "DeepSeek Platform",
    domain: "platform.deepseek.com",
    websiteUrl: "https://platform.deepseek.com/",
    description:
      "DeepSeek official API platform for DeepSeek chat and reasoning model access.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Credit card", "Alipay"],
    minimumTopUpAmount: null,
    minimumTopUpCurrency: "USD",
    supportChannels: ["Docs", "Console"],
    channelType: "official_direct",
    supportedProviders: ["DeepSeek"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: true,
    status: "published",
    riskLevel: "low",
    riskTags: ["manual-review"],
    sourceUrl: "https://api-docs.deepseek.com/quick_start/pricing",
    lastCheckedAt: "2026-05-25T00:00:00.000Z",
  },
  {
    slug: "openrouter",
    name: "OpenRouter",
    domain: "openrouter.ai",
    websiteUrl: "https://openrouter.ai/",
    description:
      "International model routing marketplace with public model pricing pages.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["VISA", "USDT"],
    minimumTopUpAmount: 5,
    minimumTopUpCurrency: "USD",
    supportChannels: ["Docs", "Discord"],
    channelType: "third_party_relay",
    supportedProviders: ["OpenAI", "Anthropic", "Google", "xAI", "Meta"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: false,
    status: "published",
    riskLevel: "unknown",
    riskTags: ["manual-review", "international-payment"],
    sourceUrl: "https://openrouter.ai/pricing",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
  {
    slug: "zhipu-bigmodel",
    name: "智谱 BigModel",
    domain: "bigmodel.cn",
    websiteUrl: "https://bigmodel.cn/",
    description:
      "Zhipu official BigModel platform for GLM model API access and domestic billing.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Alipay", "WeChat Pay", "Corporate payment"],
    minimumTopUpAmount: null,
    minimumTopUpCurrency: "CNY",
    supportChannels: ["Docs", "Console"],
    channelType: "official_direct",
    supportedProviders: ["Zhipu AI"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: true,
    status: "published",
    riskLevel: "low",
    riskTags: ["manual-review"],
    sourceUrl: "https://bigmodel.cn/pricing",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
  {
    slug: "moonshot-kimi",
    name: "Moonshot / Kimi",
    domain: "moonshot.cn",
    websiteUrl: "https://platform.moonshot.cn/",
    description:
      "Moonshot official platform for Kimi model API access and long-context workloads.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Alipay", "WeChat Pay", "Corporate payment"],
    minimumTopUpAmount: null,
    minimumTopUpCurrency: "CNY",
    supportChannels: ["Docs", "Console"],
    channelType: "official_direct",
    supportedProviders: ["Moonshot AI"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: true,
    status: "published",
    riskLevel: "low",
    riskTags: ["manual-review"],
    sourceUrl: "https://platform.moonshot.cn/docs/pricing/chat",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
  {
    slug: "volcengine-ark",
    name: "火山引擎方舟",
    domain: "volcengine.com",
    websiteUrl: "https://www.volcengine.com/product/ark",
    description:
      "Volcengine Ark official platform for Doubao and model service traffic packages.",
    billingModes: ["pay_as_you_go", "traffic_package"],
    paymentMethods: ["Alipay", "WeChat Pay", "Corporate payment"],
    minimumTopUpAmount: null,
    minimumTopUpCurrency: "CNY",
    supportChannels: ["Docs", "Console"],
    channelType: "official_direct",
    supportedProviders: ["Volcengine"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: true,
    status: "published",
    riskLevel: "low",
    riskTags: ["manual-review"],
    sourceUrl: "https://www.volcengine.com/product/ark",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
  {
    slug: "302-ai",
    name: "302.AI",
    domain: "302.ai",
    websiteUrl: "https://302.ai/",
    description:
      "AI API aggregation platform referenced in relay station directories.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Alipay", "USDT"],
    minimumTopUpAmount: 1,
    minimumTopUpCurrency: "USD",
    supportChannels: ["Website"],
    channelType: "third_party_relay",
    supportedProviders: ["OpenAI", "Anthropic", "Google"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: true,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: false,
    status: "published",
    riskLevel: "unknown",
    riskTags: ["manual-review", "referral-available"],
    sourceUrl: "https://howtok.net/",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
  {
    slug: "aihubmix",
    name: "推理时代",
    domain: "aihubmix.com",
    websiteUrl: "https://aihubmix.com/",
    description:
      "Chinese relay station candidate with low top-up threshold in public directories.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Alipay", "VISA"],
    minimumTopUpAmount: 1,
    minimumTopUpCurrency: "CNY",
    supportChannels: ["Website"],
    channelType: "third_party_relay",
    supportedProviders: ["OpenAI", "Anthropic", "Google"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: false,
    status: "published",
    riskLevel: "unknown",
    riskTags: ["manual-review"],
    sourceUrl: "https://howtok.net/",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
  {
    slug: "zenmux",
    name: "Zenmux",
    domain: "zenmux.ai",
    websiteUrl: "https://zenmux.ai/",
    description:
      "Relay station candidate with pay-as-you-go and subscription modes in public directories.",
    billingModes: ["pay_as_you_go", "subscription"],
    paymentMethods: ["WeChat Pay", "Alipay", "USDT"],
    minimumTopUpAmount: 5,
    minimumTopUpCurrency: "USD",
    supportChannels: ["Website"],
    channelType: "third_party_relay",
    supportedProviders: ["OpenAI", "Anthropic", "Google"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: false,
    status: "published",
    riskLevel: "unknown",
    riskTags: ["manual-review"],
    sourceUrl: "https://howtok.net/",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
  {
    slug: "crazyrouter",
    name: "Crazyrouter",
    domain: "crazyrouter.com",
    websiteUrl: "https://crazyrouter.com/",
    description:
      "Relay station candidate supporting multiple payment methods in public directories.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["WeChat Pay", "Alipay", "USDT", "VISA"],
    minimumTopUpAmount: 1,
    minimumTopUpCurrency: "USD",
    supportChannels: ["Website"],
    channelType: "third_party_relay",
    supportedProviders: ["OpenAI", "Anthropic", "Google", "xAI"],
    hasPublicPricing: true,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: false,
    status: "published",
    riskLevel: "unknown",
    riskTags: ["manual-review"],
    sourceUrl: "https://howtok.net/",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
  {
    slug: "unknown-sample",
    name: "Unverified Sample Relay",
    domain: "example.invalid",
    websiteUrl: "https://example.invalid/",
    description: "Draft example used to verify admin and publication filters.",
    billingModes: ["pay_as_you_go"],
    paymentMethods: ["Unknown"],
    minimumTopUpAmount: null,
    minimumTopUpCurrency: null,
    supportChannels: [],
    channelType: "third_party_relay",
    supportedProviders: [],
    hasPublicPricing: false,
    hasTrialCredit: false,
    hasReferralProgram: false,
    referralUrl: null,
    couponCode: null,
    isSponsored: false,
    isVerified: false,
    status: "draft",
    riskLevel: "high",
    riskTags: ["no-public-pricing"],
    sourceUrl: "https://example.invalid/",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
  },
];

export const relayModelPrices: RelayModelPriceFixture[] = [
  {
    relaySlug: "openai-platform",
    modelSlug: "gpt-4o-mini",
    routeName: "official",
    billingType: "token",
    modelMultiplier: null,
    completionMultiplier: null,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: 0.15,
    outputPricePer1M: 0.6,
    currency: "USD",
    sourceUrl: "https://openai.com/api/pricing/",
    lastCheckedAt: "2026-05-25T00:00:00.000Z",
    isCurrent: true,
    notes: "Official direct API pricing entry.",
  },
  {
    relaySlug: "anthropic-console",
    modelSlug: "claude-sonnet-4-6",
    routeName: "official",
    billingType: "token",
    modelMultiplier: null,
    completionMultiplier: null,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: 3,
    outputPricePer1M: 15,
    currency: "USD",
    sourceUrl: "https://docs.anthropic.com/en/docs/about-claude/pricing",
    lastCheckedAt: "2026-05-25T00:00:00.000Z",
    isCurrent: true,
    notes: "Official direct API pricing entry.",
  },
  {
    relaySlug: "google-gemini-api",
    modelSlug: "gemini-2-5-flash",
    routeName: "official",
    billingType: "token",
    modelMultiplier: null,
    completionMultiplier: null,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: 0.3,
    outputPricePer1M: 2.5,
    currency: "USD",
    sourceUrl: "https://ai.google.dev/gemini-api/docs/pricing",
    lastCheckedAt: "2026-05-25T00:00:00.000Z",
    isCurrent: true,
    notes: "Official direct API pricing entry.",
  },
  {
    relaySlug: "deepseek-platform",
    modelSlug: "deepseek-v4-flash",
    routeName: "official",
    billingType: "token",
    modelMultiplier: null,
    completionMultiplier: null,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: 0.14,
    outputPricePer1M: 0.28,
    currency: "USD",
    sourceUrl: "https://api-docs.deepseek.com/quick_start/pricing",
    lastCheckedAt: "2026-05-25T00:00:00.000Z",
    isCurrent: true,
    notes: "Official direct API pricing entry.",
  },
  {
    relaySlug: "openrouter",
    modelSlug: "gpt-4o-mini",
    routeName: "default",
    billingType: "token",
    modelMultiplier: null,
    completionMultiplier: null,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: 0.15,
    outputPricePer1M: 0.6,
    currency: "USD",
    sourceUrl: "https://openrouter.ai/pricing",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
    isCurrent: true,
    notes: "Fixture example based on public pricing page.",
  },
  {
    relaySlug: "zhipu-bigmodel",
    modelSlug: "glm-4-5",
    routeName: "official",
    billingType: "token",
    modelMultiplier: null,
    completionMultiplier: null,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: 0.8,
    outputPricePer1M: 2.4,
    currency: "USD",
    sourceUrl: "https://bigmodel.cn/pricing",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
    isCurrent: true,
    notes: "Manual planning estimate. Verify before production use.",
  },
  {
    relaySlug: "moonshot-kimi",
    modelSlug: "kimi-k2",
    routeName: "official",
    billingType: "token",
    modelMultiplier: null,
    completionMultiplier: null,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: 0.6,
    outputPricePer1M: 2.5,
    currency: "USD",
    sourceUrl: "https://platform.moonshot.cn/docs/pricing/chat",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
    isCurrent: true,
    notes: "Manual planning estimate. Verify before production use.",
  },
  {
    relaySlug: "volcengine-ark",
    modelSlug: "doubao-seed-1-6",
    routeName: "official",
    billingType: "token",
    modelMultiplier: null,
    completionMultiplier: null,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: 0.3,
    outputPricePer1M: 1.2,
    currency: "USD",
    sourceUrl: "https://www.volcengine.com/product/ark",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
    isCurrent: true,
    notes: "Manual planning estimate. Verify before production use.",
  },
  {
    relaySlug: "302-ai",
    modelSlug: "gpt-4o-mini",
    routeName: "one-api-compatible",
    billingType: "multiplier",
    modelMultiplier: 0.075,
    completionMultiplier: 4,
    groupMultiplier: 1,
    routeMultiplier: 1,
    inputPricePer1M: null,
    outputPricePer1M: null,
    currency: "USD",
    sourceUrl: "https://howtok.net/",
    lastCheckedAt: "2026-05-21T00:00:00.000Z",
    isCurrent: true,
    notes: "Manual planning estimate. Verify before production use.",
  },
];
